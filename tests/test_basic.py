...
class BasicTest(unittest.TestCase):
    def test_find(self):
        ...
        ut = dict(little.module_loader.find(TEST_MODULE, lambda x: x.startswith("find_")))
    ...
    def test_class_finder_default_predicate(self):
        ...
        ut = dict(little.module_loader.class_finder(TEST_MODULE, TestParent))
        self.assertTrue(len(ut)>0)
        self.assertIsInstance(ut, types.DictType)
    ...
    def test_function_finder_default_predicate(self):
        ...
        ut = dict(little.module_loader.function_finder(TEST_MODULE, 'main'))
        self.assertTrue(len(ut)>0)
